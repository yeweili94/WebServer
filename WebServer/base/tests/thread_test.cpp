...
class Foo
{
 public:
  ...
  void memberFunc()
  {
    printf("tid=%d, Foo::x_=%f\n", ywl::CurrentThread::tid(), x_);
  }

  void memberFunc2(const std::string& text)
  {
    printf("tid=%d, Foo::x_=%f, text=%s\n", ywl::CurrentThread::tid(), x_, text.c_str());
  }
...
int main()
{
  printf("pid=%d, tid=%d\n", ::getpid(), ywl::CurrentThread::tid());

  ywl::Thread t1(threadFunc);
  t1.start();
  t1.join();
